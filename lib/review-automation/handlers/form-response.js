...
const { isFormWebhook } = require('../auth');
const { findBooking } = require('../booking-match');
const { getSupabase } = require('../clients');
const { issueCoupon } = require('../coupons');
const { sendCoupon, sendFeedbackShareLink, sendReviewRequest } = require('../messaging');
const {
  cleanText, json, normalizeEmail, readJson, requireMethod
} = require('../http');

function parseRating(value) {
  // Only accept a complete rating value.  This intentionally rejects values
  // such as "5 or more" rather than extracting the first digit from them.
  const match = String(value ?? '')
    .trim()
    .match(/^([1-5])(?:\s*\/\s*5)?(?:\s*stars?)?$/i);
...
async function handler(req, res) {
  ...
  try {
    ...
    // Coupon eligibility is intentionally exact: only a 5/5 response may receive a coupon.
    // Do not use a configurable minimum here; a value of 1 would make every rating eligible.
    const couponEligible = rating === 5;
    ...
    if (process.env.REVIEW_FEEDBACK_SHARE_MODE === 'all_respondents') {
      try {
        const sent = await sendFeedbackShareLink(reward);
        if (!sent.skipped) {
          const { data } = await supabase.from('review_rewards').update({
            request_sent_at: new Date().toISOString(), last_error: null
          }).eq('id', reward.id).select('*').single();
          reward = data || reward;
        }
      } catch (error) {
        ...
        console.error('Feedback share email failed:', error);
      }
    }
...
module.exports.couponEligibleForRating = value => parseRating(value) === 5;
module.exports.parseRating = parseRating;
