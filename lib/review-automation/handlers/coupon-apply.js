...
const { isSupportedReviewCouponDiscount, paymentContactMatches } = require('../coupons');
const {
...
async function handler(req, res) {
  ...
  try {
    ...
    if (paymentIntent.metadata?.review_coupon_reservation_token === reservationToken) {
      const existingDiscount = Number(paymentIntent.metadata.review_coupon_discount_cents);
      if (!isSupportedReviewCouponDiscount(existingDiscount)) throw new Error('INVALID_COUPON');
      return json(res, 200, {
        ok: true, applied: true, paymentIntentId, finalAmount: paymentIntent.amount,
        discountAmount: existingDiscount, idempotent: true
      });
    }
    ...
    const discountAmount = Number(reservation?.discount_amount);
    if (!reservation || !isSupportedReviewCouponDiscount(discountAmount)) throw new Error('INVALID_COUPON');
    if (paymentIntent.amount < discountAmount) throw new Error('DISCOUNT_EXCEEDS_AMOUNT');
    ...
    const finalAmount = paymentIntent.amount - discountAmount;
    await stripe.paymentIntents.update(paymentIntentId, {
      amount: finalAmount,
      metadata: {
        review_reward_id: reservation.reward_id,
        review_coupon_code: couponCode,
        review_coupon_discount_cents: String(discountAmount),
