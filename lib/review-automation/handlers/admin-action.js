...
function isExactlyFiveStars(reward) {
  return Number(reward?.internal_rating) === 5;
}
...
async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'observe_external_review') {
      ...
      // This is a hard business rule across every issuance path.  Do not allow
      // a manual `force` flag or an environment minimum to bypass it.
      if (!isExactlyFiveStars(reward)) {
        return json(res, 409, { error: 'INTERNAL_FEEDBACK_NOT_ELIGIBLE' });
      }
      reward = await issueCoupon(supabase, id, {
        reason: body.reason || 'internal_feedback', force: false
      });
...
module.exports.isExactlyFiveStars = isExactlyFiveStars;
