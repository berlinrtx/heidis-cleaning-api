...
const { rewardDiscountCents } = require('./coupons');
const { cleanText } = require('./http');
const { createFeedbackShareUrl } = require('./share-links');

let cachedCouponHeaderAttachment;
const FEEDBACK_EMAIL_DELAY_MS = 2 * 60 * 1000;

async function sendEmail({ to, subject, html, text, attachments, scheduledAt }) {
...
function reviewEmailHeader(includeHeaderImage) {
  return includeHeaderImage
...
function buildCouponEmail(reward, { expires, includeHeaderImage = true }) {
  const name = escapeHtml(reward.customer_name || 'there');
  const code = escapeHtml(reward.coupon_code);
  const amount = rewardDiscountCents(reward) / 100;
  const amountLabel = `$${amount.toFixed(0)}`;
  ...
  const header = reviewEmailHeader(includeHeaderImage);

  return `
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">Your ${amountLabel} Heidi's Cleaning coupon code is ${code}.</div>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:640px;background:#ffffff;border:1px solid #d7e7f3;border-radius:18px;border-collapse:separate;overflow:hidden;">
            ...
            <tr>
              <td align="center" style="padding:30px 24px 34px;">
                <p style="margin:0 0 8px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:26px;line-height:1.35;color:#12243b;font-weight:700;">Hi ${name},</p>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#edf6fc" style="width:100%;max-width:530px;background:#edf6fc;border:1px solid #c7e2f3;border-radius:18px;border-collapse:separate;overflow:hidden;">
                  ...
                  <tr>
                    <td align="center" style="padding:0 20px;">
                      <p style="margin:0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:58px;line-height:1.05;color:#33a8dc;font-weight:700;letter-spacing:-2px;">${amountLabel}</p>
...
function buildFeedbackShareEmail(reward, { shareUrl, includeHeaderImage = true }) {
  const name = escapeHtml(reward.customer_name || 'there');
  const safeShareUrl = escapeHtml(shareUrl);
  const header = reviewEmailHeader(includeHeaderImage);

  return `
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">Review, edit, or copy your Heidi's Cleaning feedback before choosing Google or Yelp.</div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;background:#edf6fc;border-collapse:collapse;font-family:Poppins,Arial,Helvetica,sans-serif;color:#12243b;">
      <tr>
        <td align="center" style="padding:32px 14px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:640px;background:#ffffff;border:1px solid #d7e7f3;border-radius:18px;border-collapse:separate;overflow:hidden;">
            ...
            <tr>
              <td align="center" style="padding:30px 24px 34px;">
                <p style="margin:0 0 8px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:26px;line-height:1.35;color:#12243b;font-weight:700;">Hi ${name},</p>
                <h1 style="margin:0 0 10px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:28px;line-height:1.25;color:#214e78;font-weight:700;">Thank you for your feedback!</h1>
                <p style="margin:0 auto 24px;max-width:510px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:16px;line-height:1.6;color:#52677d;">Your experience helps us continue improving the service we provide to every customer.</p>

                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#edf6fc" style="width:100%;max-width:530px;background:#edf6fc;border:1px solid #c7e2f3;border-radius:18px;border-collapse:separate;overflow:hidden;">
                  <tr>
                    <td align="center" style="padding:27px 22px 8px;">
                      <span style="display:inline-block;padding:6px 13px;background:#f693bd;border-radius:999px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:11px;line-height:1.2;color:#12243b;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;">Share your experience</span>
                    </td>
                  </tr>
                  <tr>
                    <td align="center" style="padding:5px 28px 0;">
                      <h2 style="margin:0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:24px;line-height:1.35;color:#214e78;font-weight:700;">Would you like to share it publicly?</h2>
                      <p style="margin:10px auto 0;max-width:440px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;line-height:1.65;color:#52677d;">You can review, edit, or copy your feedback first, then choose Google or Yelp.</p>
                    </td>
                  </tr>
                  <tr>
                    <td align="center" style="padding:22px 22px 28px;">
                      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;">
                        <tr>
                          <td align="center" bgcolor="#12243b" style="background:#12243b;border-radius:999px;">
                            <a href="${safeShareUrl}" style="display:inline-block;padding:15px 25px;font-family:Poppins,Arial,Helvetica,sans-serif;color:#ffffff;text-decoration:none;font-size:15px;line-height:1.2;font-weight:700;border-radius:999px;">Open my feedback</a>
                          </td>
                        </tr>
                      </table>
                    </td>
                ...
                <p style="margin:23px auto 0;max-width:500px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;line-height:1.65;color:#52677d;">The private page will let you copy your comment and personally choose where you would like to share it.</p>
                <p style="margin:18px auto 0;max-width:520px;padding-top:18px;border-top:1px solid #d7e7f3;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:11px;line-height:1.6;color:#6a7f92;">Sharing publicly is optional. You may post positive, neutral, or negative feedback. No coupon, discount, purchase, or service depends on whether you post or what rating you choose.</p>
                <p style="margin:12px 0 0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:#6a7f92;">Email: service@heidis.inc &nbsp;·&nbsp; Phone: 650-248-4146</p>
              </td>
            </tr>
          </table>
...
async function sendCoupon(reward) {
  ...
  const amountLabel = `$${(rewardDiscountCents(reward) / 100).toFixed(0)}`;
  return sendEmail({
    to: reward.email,
    subject: `Your ${amountLabel} Heidi's Cleaning thank-you coupon`,
    html: buildCouponEmail(reward, {
      expires,
      includeHeaderImage: Boolean(headerAttachment)
    }),
    text: `Hi ${reward.customer_name || 'there'},\n\nThank you for completing our private service survey. Your one-time ${amountLabel} Heidi's Cleaning coupon code is ${reward.coupon_code}. Use it before ${expires}.\n\nTo schedule, email service@heidis.inc or call 650-248-4146.\n\nThis coupon is provided for completing our private survey and does not depend on any public review.`,
...
async function sendFeedbackShareLink(reward) {
  ...
  return sendEmail({
    to: reward.email,
    subject: 'Would you like to share your Heidi’s Cleaning feedback?',
    html: buildFeedbackShareEmail(reward, {
      shareUrl: rawShareUrl,
      includeHeaderImage: Boolean(headerAttachment)
    }),
    text: `Hi ${reward.customer_name || 'there'},\n\nThank you for completing our private service feedback form. If you would like to share your honest experience publicly, open ${rawShareUrl} to review, edit, or copy your feedback before choosing Google or Yelp.\n\nSharing publicly is optional. You may post positive, neutral, or negative feedback. No coupon, discount, purchase, or service depends on whether you post or what rating you choose.`,
    attachments: headerAttachment ? [headerAttachment] : undefined,
    // Resend schedules this follow-up without holding the Vercel function open.
    scheduledAt: new Date(Date.now() + FEEDBACK_EMAIL_DELAY_MS).toISOString()
  });
}
